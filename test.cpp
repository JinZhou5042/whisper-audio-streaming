...
bool read_wav_file(const std::string& filename, std::vector<float>& audio_data, int& sample_rate) {
    ...
    if (header.audio_format == 3) { // IEEE float (32-bit)
        ...
        file.read(reinterpret_cast<char*>(audio_data.data()), num_samples * sizeof(float));
    } else if (header.bits_per_sample == 16) { // PCM 16-bit
        // Read as int16_t and convert to float
        std::vector<int16_t> pcm_data(num_samples);
        file.read(reinterpret_cast<char*>(pcm_data.data()), num_samples * sizeof(int16_t));
    ...
    double duration = static_cast<double>(audio_data.size()) / sample_rate;
    std::cout << "Read " << audio_data.size() << " audio samples (" 
...
int main(int argc, char** argv) {
    ...
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " <whisper_model_path> <wav_file_path> [num_threads]" << std::endl;
        return 1;
    }
    ...
    std::cout << "Model loaded in " << model_load_time.count() << " seconds" << std::endl;
    ...
    double audio_duration = static_cast<double>(audio_data.size()) / sample_rate;

    // Set up whisper parameters
    whisper_full_params wparams = whisper_full_default_params(WHISPER_SAMPLING_GREEDY);
    wparams.print_realtime   = false;
    wparams.print_progress   = false;  // Disable progress printing for cleaner output
    ...
    std::cout << "\nRunning Whisper inference with " << num_threads << " threads..." << std::endl;
    ...
    double rtf = inference_time.count() / audio_duration;

    // Print latency information
    std::cout << "\nLatency Measurements:" << std::endl;
    std::cout << "------------------------" << std::endl;
    std::cout << "Audio duration: " << std::fixed << std::setprecision(3) << audio_duration << " seconds" << std::endl;
    std::cout << "Inference time: " << inference_time.count() << " seconds" << std::endl;
    std::cout << "Real-time factor (RTF): " << std::fixed << std::setprecision(3) << rtf << "x" << std::endl;
    std::cout << "------------------------" << std::endl;
    ...
    for (int i = 0; i < n_segments; ++i) {
        ...
        std::cout << "[" << (t0 / 100) / 10.0 << "s -> " << (t1 / 100) / 10.0 << "s] " << text << std::endl;
        full_text += text;
    }
    
    std::cout << "\nFull text: " << full_text << std::endl;
